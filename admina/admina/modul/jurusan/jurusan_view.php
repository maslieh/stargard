        ...
        <div class="col-xs-12">
            <div class="box">
                ...
                <div class="box-body table-responsive">
                    <button class="btn btn-primary btn-flat down_jurusan"><i class="fa fa-cloud-download"></i> Download
                        Data Jurusan/Prodi Dari Feeder</button>
                    <p></p>
                    <table id="dtb_manual" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            <?php
                            $dtb = $db->query(
                                'select jurusan.kode_jurusan,jurusan.nama_jurusan,jurusan.id,status,jenjang from jurusan'
                            );
                            $i = 1;
                            foreach (
                                $dtb
                                ...
                                <td>
                                    ...
                                    <?= $role_act['up_act'] == 'Y'
                                        ...
                                        : '' ?>
                                    <?= $role_act['del_act'] == 'Y'
                                        ? '<button class="btn btn-danger hapus btn-flat" data-uri="' .
                                            base_admin() .
                                            'modul/jurusan/jurusan_action.php" data-id="' .
                                            $isi->id .
...
function displayError(data) {
    ...
    $output = $(
        '<div class="alert alert-danger" role="alert"><strong>Oh no!</strong> Something went wrong, please try again.</div><p>Server message: <pre><code>' +
        msg + '</code></pre></p>');
...
function checkProgress(createStatusBars) {
    ...
    if ((n - window.lastUpdate) > window.pollingPeriod || window.lastData == null) {
        ...
        data.stage.completeItems = Math.min((data.stage.completeItems + Math.floor(((new Date().getTime() / 1000) - data
            .stage.curTime) * data.stage.rate * 0.5)), data.stage.totalItems);
...
function updateDisplay(data) {
    ...
    if (window.prevpc === data.stage.pcComplete & data.stage.rate !== null) {
        data.stage.completeItems = Math.min((data.stage.completeItems + Math.floor(((new Date().getTime() / 1000) - data
            .stage.curTime) * data.stage.rate)), data.stage.totalItems);
        data.stage.pcComplete = Math.min(((data.stage.completeItems) / data.stage.totalItems), 1);
        data.stage.timeRemaining = (data.stage.totalItems - data.stage.completeItems) / data.stage.rate;
    } else {
    ...
    $output = $('<div>');
    $output.append($('<h4>' + Math.ceil((((data.stage.stageNum - 1) * 100) / (data.totalStages)) + (data.stage
        .pcComplete * 100 / (data.totalStages))) + '% complete</h4>'));
